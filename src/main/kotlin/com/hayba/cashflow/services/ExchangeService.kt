...
import com.hayba.cashflow.config.CryptoUtil
import com.hayba.cashflow.entities.QuoteMapper
import com.hayba.cashflow.entities.UserDID
import com.hayba.cashflow.exceptions.OfferingNotFoundException
import com.hayba.cashflow.exceptions.QuoteNotFoundException
import com.hayba.cashflow.exceptions.RfqNotFoundException
import com.hayba.cashflow.exceptions.UserNotFoundException
import com.hayba.cashflow.models.*
import com.hayba.cashflow.repositories.*
...
class ExchangeService(
    ...
    val pfiRepository: PfiRepository
) {
    ...
    fun createRfq(rfQRequest: CreateRfQRequest): RfqCreationResponse {
        ...
        val savedRfq = rfqRepository.save(
            com.hayba.cashflow.entities.Rfq(
                id = UUID.randomUUID(),
    ...
    fun processQuote(quoteNextStep: QuoteNextStep) {
        val optionalQuote =
            quoteRepository.findByExchangeIdAndOrderStatus(exchangeId = quoteNextStep.exchangeId, orderStatus = "")
    ...
    fun existingQuotes(didUri: String): List<com.hayba.cashflow.entities.Quote> {
        val pageable = PageRequest.of(0, 5, by("createdAt").descending())
        val page = quoteRepository.findAllByCustomerDIDAndQuoteStatusIn(didUri, listOf(QuoteStatus.PENDING), pageable)
        return page.content
    }

    private fun closeExchange(quote: com.hayba.cashflow.entities.Quote, reason: String) {
    ...
    fun orderHistory(didUri: String): List<com.hayba.cashflow.entities.Quote> {
        val pageable = PageRequest.of(0, 5, by("createdAt").descending())
        val page = quoteRepository
            .findAllByCustomerDIDAndQuoteStatusIn(
                didUri,
                listOf(QuoteStatus.COMPLETED),
                pageable
            )
        return page.content
    }
    ...
    private fun createOrder(quote: com.hayba.cashflow.entities.Quote) {
    ...
    private fun handlePostRFQCreationActions(
        externalRfq: Rfq, savedRfq: com.hayba.cashflow.entities.Rfq,
        ...
        retryTemplate.execute(
            RetryCallback<Unit, RuntimeException> {
                ...
                rfqRepository.updateExchangeStatus(
                    externalRfq.metadata.exchangeId,
    ...
    private fun processOrderStatus(orderStatus: OrderStatus) {
        ...
        if (quote.orderStatus.lowercase() == "success") {
            ...
            quoteRepository.updateQuoteStatus(quote.exchangeId, QuoteStatus.COMPLETED, LocalDateTime.now())
        }
    }
    ...
    private fun isValidQuote(_quote: Quote): Boolean {
        ...
        val pfi = pfiRepository.findByDidUri(_quote.metadata.from)
        if (optionalQuote.isEmpty) {
            quoteRepository.save(QuoteMapper.mapQuote(_quote, pfi?.name ?: ""))
